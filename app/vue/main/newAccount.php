    ...
    <form class="fondCanard" method="POST" action="?path=register">
        <div class="fields-box">
            ...
            <div class="field fondVertClair">
                ...
                <input type="text" name="pseudonym" placeholder="Votre psedo"
                    <?php if (!empty($_POST["pseudonym"])): ?>
                    value=<?= $_POST["pseudonym"] ?>
                    <?php endif ?>>
                <?php
                if (!empty($datas['errors']['pseudonym'])) {
                    foreach ($datas['errors']['pseudonym'] as $data) {
                        echo ("<br><span class='error'>" . $data . "</span>");
                    }
                } ?>
            </div>
